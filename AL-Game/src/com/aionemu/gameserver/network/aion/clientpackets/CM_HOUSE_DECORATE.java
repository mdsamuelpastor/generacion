...
public class CM_HOUSE_DECORATE extends AionClientPacket {
	...
	protected void runImpl() {
		...
		if (objectId == 0) {
			...
			int sameItems = house.getRegistry().getCustomPartCountByPartId(templateId);
			if (sameItems > 1) {
				if (decor.getPersistentState() == PersistentState.NEW)
					house.getRegistry().discardPart(decor);
				else {
					decor.setPersistentState(PersistentState.DELETED);
				}
				decor = house.getRegistry().getCustomPartByPartId(templateId);
			}
