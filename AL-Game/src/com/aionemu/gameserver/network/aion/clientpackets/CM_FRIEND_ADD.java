...
public class CM_FRIEND_ADD extends AionClientPacket {
	...
	protected void runImpl() {
		...
		PacketSendUtility.sendMessage(activePlayer, "PlayerName: "+ targetName+" Message: "+message);
		...
		else // Send request
		{
			...
			boolean requested = targetPlayer.getResponseRequester().putRequest(
				SM_QUESTION_WINDOW.STR_BUDDYLIST_ADD_BUDDY_REQUEST_NEW, responseHandler);
			...
			if (!requested) {
				...
				targetPlayer.getClientConnection().sendPacket(new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_BUDDYLIST_ADD_BUDDY_REQUEST_NEW, activePlayer.getObjectId(), 0, activePlayer.getName(),new DescriptionMsg(message)));
			}
		}
	}
