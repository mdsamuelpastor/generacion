...
import com.aionemu.gameserver.model.gameobjects.BattleGroundFlag;
import com.aionemu.gameserver.model.gameobjects.BattleGroundFlag.BattleGroundFlagState;
import com.aionemu.gameserver.model.gameobjects.player.Player;
import com.aionemu.gameserver.model.templates.BattleGroundTemplate;
import com.aionemu.gameserver.model.templates.ObjectLocation;
...
public class BattleGroundFlagController extends NpcController
{
	...
	private void onFlagCaptured(Player carrier)
	{
		...
		carrier.getBattleGround().broadcastToBattleGround(
			carrier.getCommonData().getName() + (carrier.getLegion() != null ? " of " + carrier.getLegion().getLegionName() : "") + " has captured the "
	...
	public void onDialogRequest(final Player player)
	{
		...
		if(player.getBattleGround() != null && player.getBattleGround().running)
		{
			...
			else if(dropped && player.getCommonData().getRace() == getOwner().getRace())
			{
				...
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDFLAGCONTROLLER_2));
		}
	}
