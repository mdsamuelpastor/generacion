...
/**
 * @author ATracer
 * @modified Wakizashi, Source, vlog, Maestross
 */
public class EnchantService {
	...
	public static void enchantItemAct(Player player, Item parentItem, Item targetItem, Item supplementItem, int currentEnchant, boolean result) {
		...
		float random2 = Rnd.get(1, 95);
		// Items that are Fabled or Eternal can get up to +15.
		player.updateSupplements();
		if (result) {
		
			// For test purpose. To use by administrator
			if (player.getAccessLevel() > 2 && random2 > 90) {
				PacketSendUtility.sendMessage(player, "RndBonusEnchant: " + random2 + " > 90, gain +3 enchant");
			} else if (player.getAccessLevel() > 2 && (random2 < 90 && random2 > 83)) {
				PacketSendUtility.sendMessage(player, "RndBonusEnchant: 83 < " + random2 + " < 90, gain +2 enchant");
			}
			...
			switch (targetQuality) {
				...
				case LEGEND:
					...
					//3.9, with one stone and with chance, your enchant can take +2/+3 in one time
					else if(random2 > 90 && currentEnchant < 11) //5% chance						
						currentEnchant += 3;
					else if((random2 < 90 && random2 > 83) && currentEnchant < 11) //7% chance
						currentEnchant += 2;
					else
						currentEnchant += 1;
					break;
