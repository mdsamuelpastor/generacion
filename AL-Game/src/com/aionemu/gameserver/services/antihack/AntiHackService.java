	...
	public static boolean canMove(Player player, float x, float y, float z, float speed, byte type) {
			...
			if (type != 0) {
				if ((type == -64) || (type == -128)) {
					...
					if (vector2D != 0) {
						...
						if (player.speedHackCounter > SecurityConfig.SPEEDHACK_COUNTER) {
							return punish(player, x, y, type, forcedMove, "Detected illegal action (Speed Hack) SHC:" + player.speedHackCounter + " S:" + speed + " V:" + Math.rint(1000 * vector2D) / 1000
								+ " type:" + type);
						}
					...
					if ((type & 0xFFFFFFC0) == -64) {
						...
						if ((player.getTarget() != null) && (player.getTarget() != player)) {
							PlayerMoveController m = player.getMoveController();
							double distDiff = MathUtil.getDistance(player.getTarget().getX(), player.getTarget().getY(), m.getTargetX2(), m.getTargetY2());
					...
					if ((SecurityConfig.PUNISH > 0) && (player.speedHackCounter > SecurityConfig.SPEEDHACK_COUNTER + 5)) {
						return punish(player, x, y, type, forcedMove, "Detected illegal action (Speed Hack) SHC:" + player.speedHackCounter + " SMS:" + Math.rint(100 * (timeDiff * (speed + 0.25) * 0.001)) / 100
							+ " TDF:" + timeDiff + " VTD:" + Math.rint(1000 * (timeDiff * (speed + 0.85) * 0.001)) / 1000 + " VS:" + Math.rint(100 * vector) / 100 + " type:" + type);
					}
				...
				if ((SecurityConfig.PUNISH > 0) && (player.speedHackCounter > SecurityConfig.SPEEDHACK_COUNTER + 5)) {
					return punish(player, x, y, type, forcedMove,
						"Detected illegal action (Speed Hack) SHC:" + player.speedHackCounter + " TD:" + Math.rint(1000 * timeDiff) / 1000 + " VTD:" + Math.rint(1000 * (timeDiff * speed * 0.00075)) / 1000
							+ " VS:" + Math.rint(100 * vector) / 100 + " type:" + type);
				}
		...
		if (SecurityConfig.TELEPORTATION) {
			...
			if ((speed > 5d) && (delta > 5) && ((type & 0x4) != 4)) {
				...
				return punish(player, x, y, type, normalMove, "Detected illegal action (Teleportation) S:" + speed + " D:" + Math.rint(1000 * delta) / 1000 + " type:" + type);
			}
	...
	protected static boolean punish(Player player, float x, float y, byte type, AionServerPacket pkt, String message) {
		switch (SecurityConfig.PUNISH) {
			...
			case 2:
				AuditLogger.info(player, message);
				moveBack(player, x, y, type, pkt);
				if ((player.speedHackCounter > SecurityConfig.SPEEDHACK_COUNTER * 6) || (player.abnormalHackCounter > SecurityConfig.ABNORMAL_COUNTER * 4)) {
