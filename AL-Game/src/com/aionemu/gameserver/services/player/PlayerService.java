...
public class PlayerService {
	...
	public static boolean storeNewPlayer(Player player, String accountName, int accountId) {
		return DAOManager.getDAO(PlayerDAO.class).saveNewPlayer(player.getCommonData(), accountId, accountName) && DAOManager.getDAO(PlayerAppearanceDAO.class).store(player)
			&& DAOManager.getDAO(PlayerSkillListDAO.class).storeSkills(player) && DAOManager.getDAO(InventoryDAO.class).store(player);
	}
	...
	public static void storePlayer(Player player) {
		...
		for (House house : player.getHouses()) {
			...
			if ((house.getRegistry() != null) && (house.getRegistry().getPersistentState() == PersistentState.UPDATE_REQUIRED)) {
				DAOManager.getDAO(PlayerRegisteredItemsDAO.class).store(house.getRegistry(), player.getCommonData().getPlayerObjId(), house.getAddress().getId());
			}
		}
