...
public class PlayerLeaveWorldService {
	...
	public static final void startLeaveWorld(Player player) {
	    // if player have other race as elyos or asmodian, get saved race and set it to this race
	    if (player.getRace() != Race.ELYOS && player.getRace() != Race.ASMODIANS) {
		    player.getCommonData().setRace(player.getCommonData().getRace());
		}
