...
public class BrokerService {
	...
	public void buyBrokerItem(Player player, int itemUniqueId) {
		...
		if (LoggingConfig.LOG_BROKER_EXCHANGE)
			log.info("[BROKER EXCHANGE] > [Player: " + player.getName() + "] bought [Item: " + buyingItem.getItemId() + "] " + "[Count: " + buyingItem.getItemCount()
				+ (LoggingConfig.ENABLE_ADVANCED_LOGGING ? "] [Item Name: " + item.getItemName() : "]") + " from [Player: " + buyingItem.getSeller() + "] for [Price: " + buyingItem.getPrice() + "]");
		...
		showRequestedItems(player, getPlayerCache(player).getBrokerMaskCache(), getPlayerCache(player).getBrokerSortTypeCache(), getPlayerCache(player).getBrokerStartPageCache(), getPlayerCache(player)
			.getSearchItemList());
	}
