...
public class DropService {
	...
	public boolean canAutoLoot(Player player, DropItem requestedItem) {
		int npcId = requestedItem.getNpcObj();
		DropNpc dropNpc = DropRegistrationService.getInstance().getDropRegistrationMap().get(Integer.valueOf(npcId));
		if (dropNpc == null) {
			return false;
		}
	...
	public void requestDropItem(Player player, int npcId, int itemIndex, boolean autoLoot) {
		...
		if ((requestedItem.getDropTemplate().getItemTemplate().hasLimitOne())
			&& ((player.getInventory().getFirstItemByItemId(itemId) != null) || (player.getStorage(StorageType.REGULAR_WAREHOUSE.getId()).getFirstItemByItemId(itemId) != null))) {
			PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_CAN_NOT_GET_LORE_ITEM("[item:" + itemId + "]"));
			return;
		}
