	...
	public static void finishCrafting(final Player player, RecipeTemplate recipetemplate, int critCount, int bonus) {
		...
		if (LoggingConfig.LOG_CRAFT) {
			log.info(new StringBuilder()
				.append(critCount > 0 ? "[CRAFT][Critical] ID/Count" : "[CRAFT][Normal] ID/Count")
				.append(
					LoggingConfig.ENABLE_ADVANCED_LOGGING ? new StringBuilder().append("/Item Name - ").append(productItemId).append("/").append(recipetemplate.getQuantity()).append("/")
						.append(itemTemplate.getName()).toString() : new StringBuilder().append(" - ").append(productItemId).append("/").append(recipetemplate.getQuantity()).toString()).append(" to player ")
				.append(player.getName()).toString());
		}

		if (player.getSkillList().addSkillXp(player, recipetemplate.getSkillid(), (int) RewardType.CRAFTING.calcReward(player, xpReward), recipetemplate.getSkillpoint())) {
		...
		if (recipetemplate.getCraftDelayId() != null) {
			player.getCraftCooldownList().addCraftCooldown(recipetemplate.getCraftDelayId(), recipetemplate.getCraftDelayTime() / AdvCustomConfig.CRAFT_DELAYTIME_RATE);
		}
	}
