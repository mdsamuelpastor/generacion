...
public class SkillLearnService {
	...
	private static void addSkills(Player player, int level, PlayerClass playerClass, Race playerRace) {
		...
		for (SkillLearnTemplate template : skillTemplates) {
			...
			if (player.getActiveHouse() != null && playerSkillList.getSkillEntry(2670) != null || playerSkillList.getSkillEntry(2671) != null) {
				if (player.getRace() == Race.ASMODIANS) {
					playerSkillList.addSkill(player, 2671, 1);
				} else {
					playerSkillList.addSkill(player, 2670, 1);
				}
			}
		}
	}
