...
/**
 * @author Mr. Poke
 * @modified vlog, bobobear, xTz, Rolandas, Maestross, GoodT
 * @modified -Enomine-
 */
public final class QuestService {
	...
	private static boolean giveRewardAndFinish(QuestEnv env, QuestTemplate template, boolean extended, int reward) {
		...
		if (dialogId != 18 && dialogId != 0 && !extended) {
			...
			if (index - 8 >= 0 && index - 8 < rewards.getSelectableRewardItem().size()) {
				...
				log.error("The extended SelectableRewardItem list has no element with the given index (extendedRewardIndex - 8) of " + (index - 8) + ". See quest id " + env.getQuestId() + ". The size is: "
	...
	private static boolean setFinishingState(QuestEnv env, QuestTemplate template, int reward) {
		...
		if (template.getCategory() == QuestCategory.LTASK) {
			ChallengeTaskService.updateTask(env.getQuestId(), player.getLegion(), env.getPlayer());
		}
	...
	private static boolean checkStartConditionsImpl(QuestEnv env, boolean warn) {
		...
		if (template.getCombineSkill() != null) {
			...
			{
				skills.add(30002); //Vitality
				skills.add(30003); //Aether
				skills.add(40001); //Cooking
				skills.add(40002); //Weaponsmithing
				skills.add(40003); //Armorsmithing
				skills.add(40004); //Tailoring
				skills.add(40007); //Alchemy
				skills.add(40008); //Handicrafting
				skills.add(40010); //Menusier
			}
			else {
