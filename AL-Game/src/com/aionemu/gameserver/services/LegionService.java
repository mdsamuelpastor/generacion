...
public class LegionService {
	 ...
	 */
	private static final int MAX_LEGION_LEVEL = 8;//Aion 3.5 retail like
	...
	private void invitePlayerToLegion(final Player activePlayer, final Player targetPlayer) {
		if (legionRestrictions.canInvitePlayer(activePlayer, targetPlayer)) {
			...
			if (!requested) {
				...
				PacketSendUtility.sendPacket(targetPlayer, new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_GUILD_INVITE_DO_YOU_ACCEPT_INVITATION, 0, 0, legion.getLegionName(), legion.getLegionLevel() + "",
				...
				public void acceptRequest(Creature requester, Player responder) {
					if (!targetPlayer.getCommonData().isOnline()) {
						...
						if (legionMember.getRank().getRankId() > LegionRank.BRIGADE_GENERAL.getRankId()) {
							...
							PacketSendUtility.broadcastPacketToLegion(legion, new SM_LEGION_EDIT(8));
	...
	private void updateMembersEmblem(Legion legion, LegionEmblemType emblemType) {
		...
		for (Player onlineLegionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.broadcastPacket(onlineLegionMember, new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(),
	...
	public void storeLegionEmblem(Player activePlayer, int legionId, int emblemId, int color_r, int color_g, int color_b, LegionEmblemType emblemType) {
		if (legionRestrictions.canStoreLegionEmblem(activePlayer, legionId, emblemId)) {
			...
			PacketSendUtility.sendPacket(activePlayer, new SM_LEGION_UPDATE_EMBLEM(legionId, emblemId, color_r, color_g, color_b, emblemType));
			PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_GUILD_CHANGE_EMBLEM);
		}
	}
	 ...
	 * @param pointsGained
	 */
	public void addContributionPoints(Legion legion, long pointsGained) {
	...
	public void uploadEmblemData(Player activePlayer, int size, byte[] data) {
		if (legionRestrictions.canUploadEmblem(activePlayer)) {
			...
			if (legionEmblem.getUploadSize() == legionEmblem.getUploadedSize()) {
				...
				if (!activePlayer.getInventory().tryDecreaseKinah(1130000L)) {
	...
	public void sendEmblemData(Player player, LegionEmblem legionEmblem, int legionId, String legionName) {
		PacketSendUtility.sendPacket(player, new SM_LEGION_SEND_EMBLEM(legionId, legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), legionName,
			legionEmblem.getEmblemType(), legionEmblem.getCustomEmblemData().length));
	...
	public void setLegionName(Legion legion, String newLegionName, boolean save) {
		...
		for (Player legionMember : legion.getOnlineLegionMembers()) {
			PacketSendUtility.broadcastPacket(legionMember, new SM_LEGION_UPDATE_TITLE(legionMember.getObjectId(), legion.getLegionId(), legion.getLegionName(), legionMember.getLegionMember().getRank()
				.getRankId()), true);
		}
		if (save)
			storeLegion(legion);
	...
	private void addLegionMember(Legion legion, Player player, LegionRank rank) {
		...
		PacketSendUtility.broadcastPacket(player,
			new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(), legionEmblem.getEmblemId(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), legionEmblem.getEmblemType()),
			true);
	...
	public void addWHItemHistory(Player player, int itemId, long count, IStorage sourceStorage, IStorage destStorage) {
		...
		if (legion != null) {
			...
			if (sourceStorage.getStorageType() == StorageType.LEGION_WAREHOUSE) {
				getInstance().addHistory(legion, player.getName(), LegionHistoryType.ITEM_WITHDRAW, 2, description);
			}
			else if (destStorage.getStorageType() == StorageType.LEGION_WAREHOUSE)
				getInstance().addHistory(legion, player.getName(), LegionHistoryType.ITEM_DEPOSIT, 2, description);
		}
	}
	...
	private class LegionRestrictions {
		...
		private boolean canInvitePlayer(Player activePlayer, Player targetPlayer) {
			...
			else if (activePlayer.getRace() != targetPlayer.getRace() && !LegionConfig.LEGION_INVITEOTHERFACTION && !activePlayer.isGM()) {
				// Not Same Race
				return false;
			}
			return true;
		 ...
		 */
		private boolean canDisbandLegion(Player player, Legion legion) {
			for(FortressLocation location : SiegeService.getInstance().getFortresses().values()){
        for(OutpostLocation outpost : SiegeService.getInstance().getOutposts().values()){
          if(location.getLegionId() == legion.getLegionId() || outpost.getLegionId() == legion.getLegionId()){
              PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_CANT_DISPERSE_GULILD_HAVING_HOUSE);
              return false;
          }
        }
     }
     if (player.getController().isInCombat()){ //In general, i dont understand what means "war"
       PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_CANT_DISPERSE_WHILE_WAR);
       return false;
      }
     else if (!isBrigadeGeneral(player)) {
				PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_ONLY_MASTER_CAN_DISPERSE);
				return false;
			}
				...
				PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_ALREADY_REQUESTED);
				return false;
			}
			else if (legion.getLegionWarehouse().size() > 0) {
				PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_GUILD_DISPERSE_CANT_DISPERSE_GUILD_STORE_ITEM_IN_WAREHOUSE);
				return false;
			}
			return true;
		}
		...
		public boolean canOpenWarehouse(Player activePlayer) {
			...
			if (whUser != playerId && legWh.getWhUser() != 0) {
				PacketSendUtility.sendPacket(activePlayer, new SM_SYSTEM_MESSAGE(1300280));
				return false;
			}
			legWh.setWhUser(activePlayer.getObjectId());
			return true;
		}
