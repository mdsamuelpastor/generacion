...
public class LootGroupRules {
	...
	public LootGroupRules() 
	{
		...
		over_ethernal = 2;
		over_over_ethernal = 0;	// 4.0 or later
		misc = 0;
	}

	public LootGroupRules(LootRuleType lootRule, LootDistribution autodistribution, int commonItemAbove, int superiorItemAbove, int heroicItemAbove, int fabledItemAbove, int ethernalItemAbove , int OverEthernal , int OverOverEthernal, int misc)

	{
		super();
		this.lootRule = lootRule;
		this.lootdistribution = autodistribution;
		this.misc = misc;
	...
	public boolean getQualityRule(ItemQuality quality) 
	{
		switch(quality)
		{
			...
			case MYTHIC:
					return over_ethernal  != 0;
			//case OVER:
					//return over_over_ethernal  != 0;
			default:
				break;
		}
		return false;
