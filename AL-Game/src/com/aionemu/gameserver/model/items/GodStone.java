	...
	public void onEquip(final Player player) {
		...
		actionListener = new ActionObserver(ObserverType.ATTACK) {
			...
			public void attack(Creature creature) {
				int handProbability = Rnd.get(probability - probabilityLeft, probability);
				//half chance for offhandWeapon
				if (equippedItem == player.getEquipment().getOffHandWeapon() && player.getEquipment().isDualWieldEquipped()) {
					handProbability *= 0.5;
				}
