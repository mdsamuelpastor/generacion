...
public class UseableItemObject extends HouseObject<HousingUseableItem> {
	...
	public void onUse(final Player player) {
		...
		if (getObjectTemplate().getPlacementLimit() == LimitType.COOKING) {
			if (player.getInventory().getItemCountByItemId(action.getRewardId()) > 0L) {
		...
		int delay = getObjectTemplate().getDelay();
				...
				try {
					...
					if (useCount != null) {
						if (action.getFinalRewardId() != null && useCount + 1 == usedCount) {
							...
							ItemService.addItem(player, rewardId, 1L);
							delete = true;
						}
							...
							if (useCount == usedCount) {
								...
								if (action.getFinalRewardId() == null) {
									...
									myself.setExpireTime((int) (System.currentTimeMillis() / 1000L));
					...
					if (delete) {
						selfDestroy(player, SM_SYSTEM_MESSAGE.STR_MSG_HOUSING_OBJECT_DELETE_USE_COUNT_FINAL(myself.getObjectTemplate().getNameId()));
					}
					else {
						...
						if ((cd == null) || (cd == 0)) {
							...
							DateTime repeatDate = new DateTime(tomorrow.getYear(), tomorrow.getMonthOfYear(), tomorrow.getDayOfMonth(), 0, 0, 0);
	...
	public void expireEnd(Player player) {
		int descId = getObjectTemplate().getNameId();
		SM_SYSTEM_MESSAGE msg = SM_SYSTEM_MESSAGE.STR_MSG_HOUSING_OBJECT_DELETE_EXPIRE_TIME(descId);
