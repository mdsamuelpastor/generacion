...
public class Player extends Creature {
	...
	private Storage[] cabinets = new Storage[20];
	...
	private FastMap<Integer, Integer> improvedSkills = new FastMap<Integer, Integer>();
  private boolean isInDimensional;
  private WorldPosition activeReturn;
	...
	public void onLoggedOut() {
		...
		if (battleGround != null) {
            ...
            battleGround.broadcastToBattleGround(getName() +
            	" has left the battleground.", null);	
         //       " has left the battleground.", getCommonData().getRace());
            battleGround = null;

            if (battlegroundObserve != 0) {
                if (battlegroundBetE > 0) {
                    getStorage(StorageType.CUBE.getId())
                        .increaseKinah(battlegroundBetE);
                    battlegroundBetE = 0;
                } else if (battlegroundBetA > 0) {
                    getStorage(StorageType.CUBE.getId())
                        .increaseKinah(battlegroundBetA);
                    battlegroundBetA = 0;
                }
	...
	public QuestEnv getQuestCookie() {
		return questCookie;
	}

	public void setQuestCookie(QuestEnv questEnv, QuestEnv questCookie) {
	...
	public boolean isEnemyFrom(Player player) {
		...
		return player.getObjectId() != getObjectId()
			&& (getAdminEnmity() > 1 || player.getAdminEnmity() > 1 || isPvP(player) || getController().isDueling(player) || (isInsideZoneType(ZoneType.PVP) && player.isInsideZoneType(ZoneType.PVP)
				&& !isInSameTeam(player) && player.getWorldId() != 600020000 && player.getWorldId() != 600030000));
	}
	...
	public boolean isAggroFrom(Npc npc) {
		...
		return (isAggroIconTo(npc) && (npc.getTribe().isGuard() || npc.getObjectTemplate().getAbyssNpcType() != AbyssNpcType.NONE || npc.getLevel() + 10 > getLevel() || (npc.isInInstance() && InstanceService
			.isAggro(npc.getWorldId()))));
	}
	...
	public List<House> getHouses() {
		if (houses == null) {
			List<House> found = HousingService.getInstance().searchPlayerHouses(this);
			if (found.size() > 0)
				houses = found;
			else
				return found;
	...
	public int getHouseOwnerId() {
		...
		if (house != null) {
			return house.getAddress().getEditorId() + house.getAddress().getMapId();
		}
		return 0;
	}
	...
	public void setHousingStatus(byte status) {
		housingStatus = (byte) (housingStatus | status);
		House house = getActiveHouse();
		if (house != null)
	...
	public void unsetHousingStatus(byte status) {
		housingStatus = (byte) (housingStatus & (status ^ 0xFFFFFFFF));
		House house = getActiveHouse();
		if (house != null)
			house.fixHousingFlags();
	}
	...
	public FastMap<Integer, Integer> getImprovedSkillMap(){
    return improvedSkills;
  }

  public void addImprovedSkill(int baseSkillId, int conflictedSkillId){
    improvedSkills.put(baseSkillId, conflictedSkillId);
  }

public void setImprovedSkillsMap(FastMap<Integer, Integer> map){
    this.improvedSkills = map;
}

public WorldPosition getActiveReturn(){
  return activeReturn;
}

public void setActiveReturn(WorldPosition pos){
  this.activeReturn = pos;
}

public boolean isInDimensional(){
  return isInDimensional;
}

public void setDimensionalStatus(boolean isIn){
  this.isInDimensional = isIn;
}
    ...
    public void setBattleGround(BattleGround battleGround) {
        if (battleGround == null) {
            ...
            if (getController().getTask(TaskId.BATTLEGROUND_CARRY_FLAG) != null) {
                getController().getTask(TaskId.BATTLEGROUND_CARRY_FLAG)
                    .cancel(true);
