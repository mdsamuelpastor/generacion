...
public class VisibleObjectSpawner {
	...
	public static Kisk spawnKisk(SpawnTemplate spawn, int instanceIndex, Player creator) {
        ...
        switch (npcId) {
            ...
            case 701770:
                ...
                kisk.setVisualState(CreatureVisualState.HIDE1);
                SpawnEngine.bringIntoWorld(kisk, spawn, instanceIndex);
                PacketSendUtility.broadcastPacket(kisk, new SM_PLAYER_STATE(kisk));
                break;
            default:
                kisk.setKnownlist(new PlayerAwareKnownList(kisk));
                kisk.setCreator(creator);
	...
	public static Npc spawnPostman(Player owner) {
		...
		Vector3f pos = GeoService.getInstance().getClosestCollision(owner, owner.getX() + (float) (Math.cos(radian) * 5), owner.getY() + (float) (Math.sin(radian) * 5), owner.getZ(), false);
	...
	public static Summon spawnSummon(Player creator, int npcId, int skillId, int skillLevel, int time) {
		...
		Summon summon = new Summon(IDFactory.getInstance().nextId(), isSiegeWeapon ? new SiegeWeaponController(npcId) : new SummonController(), spawn, npcTemplate, isSiegeWeapon ? npcTemplate.getLevel()
			: level, time);
