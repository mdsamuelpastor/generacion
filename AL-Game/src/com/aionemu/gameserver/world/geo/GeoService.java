...
public class GeoService {
	...
	public boolean canSee(VisibleObject object, VisibleObject target) {
		...
		if (limit <= 0)
			return true;
		return geoData.getMap(object.getWorldId()).canSee(object.getX(), object.getY(), object.getZ() + object.getObjectTemplate().getBoundRadius().getUpper() / 2, target.getX(), target.getY(),
	...
	public Vector3f getClosestCollision(Creature object, float x, float y, float z, boolean changeDirction) {
		return geoData.getMap(object.getWorldId()).getClosestCollision(object.getX(), object.getY(), object.getZ(), x, y, z, changeDirction, object.isInFlyingState(), object.getInstanceId());
	}
