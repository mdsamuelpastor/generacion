...
public class GameTime {
	...
	/**
	 * Calculate new season time and send events on change
	 */
	public void checkSeasonTimeChange() {
		SeasonTime oldSeasonTime = this.seasonTime;
		calculateSeasonTime();
		if (oldSeasonTime != this.seasonTime) {
			onSeasonTimeChange();
		}
	}
	...
	/**
	 * Calculate the season time
	 */
	public void calculateSeasonTime() {
		int month = getMonth();
		if (month > 3 || month < 6)
			seasonTime = SeasonTime.SPRING;
		else if (month > 6)
			seasonTime = SeasonTime.SUMMER;
		else if (month > 9)
			seasonTime = SeasonTime.FALL;
		else
			seasonTime = SeasonTime.WINTER;
	}
	...
	/**
	 * Perform actions upon season time change
	 */
	private void onSeasonTimeChange() {
	    SeasonTimeSpawnEngine.onSeasonTimeChange(seasonTime);
	}
	 ...
	 * @return the dayTime
	 */
	public SeasonTime getSeasonTime() {
		return seasonTime;
	}
