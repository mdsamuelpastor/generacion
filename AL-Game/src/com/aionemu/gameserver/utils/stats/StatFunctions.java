...
public class StatFunctions {
    ...
    public static long calculateSoloGroupExperienceReward(Player player, Creature target) {
        int playerLevel = player.getCommonData().getLevel();
        int targetLevel = target.getLevel();
    ...
    public static int calculatePhysicalAttackDamage(Creature attacker, Creature target, boolean isMainHand) {
        ...
        if (attacker instanceof Player) {
            ...
            if (weapon != null) {
                ...
                if (!isMainHand) {
                    Item offWeapon = ((Player)attacker).getEquipment().getOffHandWeapon();
                    if (offWeapon.getItemTemplate().getArmorType() != ArmorType.ARROW) {
                        int dualEffect = ((Player)attacker).getDualEffectValue();
                        negativeDiff = (int)Math.round((200 - dualEffect) * 0.01 * diff);
                    }
                }
    ...
    public static int calculateMagicalSkillDamage(Creature speller, Creature target, int baseDamages, int bonus, SkillElement element, boolean useMagicBoost, boolean useKnowledge, boolean noReduce,
        int pvpDamage) {
        ...
        int mBResist = magicBoost - tgs.getMBResist().getCurrent();
        magicBoost = mBResist > 0 ? mBResist : magicBoost;
     ...
     * http://www.wolframalpha.com/input/?i=quadratic+fit+%7B%7B300%2C+30.97%7D%2C+%7B320%2C+31.68
     * %7D%2C+%7B340%2C+33.30%7D
     * %2C+%7B360%2C+36.09%7D%2C+%7B380%2C+37.81%7D%2C+%7B400%2C+40.72%7D%2C+%7B420%2C+42.12%7D%2C
     * +%7B440%2C+44.03%7D%2C+%7B480%2C+44.66%7D%2C+%7B500%2C+45.96%7D%2C%7B604%2C+51.84%7D%2C+%7B649%2C+52.69%7D%7D
