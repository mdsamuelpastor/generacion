...
public class BattleGroundManager
{
	...
	public static boolean registerPlayer(Player player, int tplId)
	{
		...
		if(player.isInGroup2())
		{
			...
			{
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE1));
				return false;
			}
				...
				if(player.getPlayerGroup2().size() <= (template.getNbPlayers() - elyosWaitList.get(tplId).size()))
				{
						...
						&& highestBgPoints <= template.getJoinConditions().getMaxBgPoints() && lowestBgPoints <= template.getJoinConditions().getRequiredBgPoints())
					{
						...
						for(int i = 0; i < members.size(); i++)
						{
							...
							PacketSendUtility.sendMessage(member[i], LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE2, template.getName()));
							PacketSendUtility.sendMessage(member[i], LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE3));
						}
						computeReadiness(tplId);
						return true;
					}
					PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE4));
					return false;
				}
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE5));
				return false;
			}
				...
				if(player.getPlayerGroup2().size() <= (template.getNbPlayers() - asmodiansWaitList.get(tplId).size()))
				{
						...
						&& highestBgPoints <= template.getJoinConditions().getMaxBgPoints() && lowestBgPoints <= template.getJoinConditions().getRequiredBgPoints())
					{
						...
						for(int i = 0; i < members.size(); i++)
						{
							member[i] = members.get(i);
							member[i].battlegroundWaiting = true;
							PacketSendUtility.sendMessage(member[i], LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE6, template.getName()));
							PacketSendUtility.sendMessage(member[i], LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE7));
						}
						computeReadiness(tplId);
						return true;
					}
					PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE8));
					return false;
				}
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE9));
				return false;
			}
			PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE10));
			return false;
		}
		if(!player.isInGroup2())
		{
				...
				|| (player.getCommonData().getRace() == Race.ASMODIANS && !asmodiansWaitList.get(tplId).contains(player)))
			{
				...
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE11, template.getName()));
				PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE12));
				computeReadiness(tplId);
				return true;
			}
			return false;
	...
	public static boolean registerPlayerObs(Player player, int tplId)
	{
		...
		PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE13, template.getName()));
		PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE14));
		return true;
	}
	...
	public static void sendRegistrationForm(Player player)
	{
		...
		{
			PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE15));
			return;
		}
		...
		// Registration form
		String html = HTMLService.HTMLTemplate(LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE16), LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE016), acceptedLocations, 0, 0);
	...
	public static void sendRegistrationFormObs(Player player)
	{
		...
		{
			PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE17));
			return;
		}
		...
		String html = HTMLService.HTMLTemplate(LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE18), LanguageHandler.translate(CustomMessageId.BATTLEGROUNDMANAGER_MESSAGE018), acceptedLoc, 0, 0);
