 ...
 * @author Rolandas
 */
public class HouseCommand extends ChatCommand {
	...
	public void execute(Player admin, String... params) {
		if (params.length == 0) {
			PacketSendUtility.sendMessage(admin, "Syntax: //house <tp | list | acquire | revoke>");
			return;
		}
		...
		if (params[0].equals("list")) {
			...
			if (houses == null || houses.size() == 0) {
				PacketSendUtility.sendMessage(admin, "There's no close houses around");
				return;
			}
			else {
				...
				for (House house : houses) {
					if (house.getOwnerId() == 0) {
						foundAny = true;
						PacketSendUtility.sendMessage(admin, house.getName());
					}
				}
				if (!foundAny)
					PacketSendUtility.sendMessage(admin, "All houses have owners");
			}
		}
		else if (params[0].equals("acquire")) {
			if (params.length == 1) {
