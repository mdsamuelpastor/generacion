...
public class TalocsHollowInstance extends GeneralInstanceHandler {
	...
	private void addItems(Player player) {
		...
		switch (player.getRace()) {
			case ELYOS:
				ItemService.addItem(player, 160001286, 1); //20min
				ItemService.addItem(player, 164000099, 1);
				break;
			case ASMODIANS:
				ItemService.addItem(player, 160001287, 1); //20min
				ItemService.addItem(player, 164000099, 1);
				break;
			default:
				break;
	...
	private void spawnRings() {
		FlyRing f1 = new FlyRing(new FlyRingTemplate("TALOCS_1", mapId, new Point3D(253.85039, 649.23535, 1171.8772), new Point3D(253.85039, 649.23535, 1177.8772), new Point3D(262.84872, 649.4091,
			1171.8772), 8), instanceId);
		f1.spawn();
		FlyRing f2 = new FlyRing(new FlyRingTemplate("TALOCS_2", mapId, new Point3D(592.32275, 844.056, 1295.0966), new Point3D(592.32275, 844.056, 1301.0966), new Point3D(595.2305, 835.5387, 1295.0966),
			8), instanceId);
		f2.spawn();
	}
	...
	public boolean onReviveEvent(Player player) {
		...
		TeleportService2.teleportTo(player, mapId, instanceId, 201.04416f, 211.99017f, 1098.9193f, (byte) 31);
		return true;
	}
