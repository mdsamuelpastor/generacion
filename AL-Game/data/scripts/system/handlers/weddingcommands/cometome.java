...
public class cometome extends ChatCommand {
	...
	public void execute(final Player player, String... params) {
		...
		if (player.getWorldId() == 510010000 || player.getWorldId() == 520010000) {
			PacketSendUtility.sendMessage(player, "Du kannst dieses Kommando im Gefaengnis nicht nutzen.");
			return;
		}
		...
		if (player.isInInstance()) {
			PacketSendUtility.sendMessage(player, "Du kannst dich nicht zu " + partner.getName() + "porten, Du bist in einer Instanz.");
			return;
		}
		...
		if (!player.isCommandInUse()) {
			...
			PacketSendUtility.sendMessage(player, partner.getName() + " zu dir teleportiert.");
