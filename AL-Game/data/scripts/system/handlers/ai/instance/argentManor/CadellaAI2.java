...
public class CadellaAI2 extends AggressiveNpcAI2 {
	...
	private List<Npc> getHelpers() {
		...
		if (npc1 != null && !CreatureActions.isAlreadyDead(npc1)) {
			helpers.add(npc1);
		}
		...
		if (npc3 != null && !CreatureActions.isAlreadyDead(npc3)) {
			helpers.add(npc3);
		}
		Npc npc4 = instance.getNpc(282348);
		if (npc4 != null && !CreatureActions.isAlreadyDead(npc4)) {
			helpers.add(npc4);
		}
		Npc npc5 = instance.getNpc(282349);
		if (npc5 != null && !CreatureActions.isAlreadyDead(npc5)) {
			helpers.add(npc5);
		}
		return helpers;
	}
