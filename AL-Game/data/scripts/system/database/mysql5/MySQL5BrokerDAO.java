...
public class MySQL5BrokerDAO extends BrokerDAO {
		...
		DB.select("SELECT * FROM inventory WHERE `item_location` = 126", new ReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					brokerItems.add(new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount, false, false, slot, location, enchant, itemSkin, fusionedItem, optionalSocket,
	...
	private boolean updateBrokerItem(final BrokerItem item) {
		boolean result = DB.insertUpdate("UPDATE broker SET `is_sold` = ?, `is_settled` = 1, `settle_time` = ? WHERE `item_pointer` = ? AND `expire_time` = ? AND `seller_id` = ? AND `is_settled` = 0",
			new IUStH() {
