		...
		DB.select("SELECT * FROM inventory WHERE `item_owner` = ? AND `item_location` = 127", new ParamReadStH() {
			...
			public void handleRead(ResultSet rset) throws SQLException {
				while (rset.next()) {
					...
					Item item = new Item(itemUniqueId, itemId, itemCount, itemColor, itemCreator, expireTime, activationCount, isEquiped == 1, isSoulBound == 1, slot, StorageType.MAILBOX.getId(), enchant,
						itemSkin, fusionedItem, optionalSocket, optionalFusionSocket, charge);
