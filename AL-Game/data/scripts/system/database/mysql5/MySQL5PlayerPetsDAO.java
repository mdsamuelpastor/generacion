...
public class MySQL5PlayerPetsDAO extends PlayerPetsDAO {
	...
	public void saveFeedStatus(Player player, int petId, int hungryLevel, int feedProgress, int feedPoints, long reuseTime) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement("UPDATE player_pets SET hungry_level = ?, feed_progress = ?, feed_points = ?, reuse_time = ? WHERE player_id = ? AND pet_id = ?");
	...
	public void insertPlayerPet(PetCommonData petCommonData) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("INSERT INTO player_pets(player_id, pet_id, decoration, name, despawn_time) VALUES(?, ?, ?, ?, ?)");
